...
static void dump(char *outfile, struct timespec **calltimes)
{
    ...
    if(popt_args.clear) {
        fprintf(file, "tracer,total call time,average call time,number of"
        ...
        for(j = 1; j < loop_per_thread; j++) {
            ...
            if(popt_args.dump) {
                fprintf(heatmap_file, "%ld,%ld\n",
                    calltimes[i][j].tv_sec * 1000000000 +
                        calltimes[i][j].tv_nsec,
    ...
    fprintf(file, "%s,%ld,%ld,%d,%d\n", popt_args.tracer, totaltime, average,
            popt_args.loop, popt_args.nthreads);
...
int main(int argc, char **argv)
{
    ...
    for(i = 0; i < nthreads; i++) {
        calltimes[i] = (struct timespec*) malloc(loops_per_thread
