...
def start_tracing(tracename, args, tracepoints = None):
    buf_size_kb = str(4 * int(args['buf_size_kb']))
    ...
    print('find ' + tracing_dir + 'events/syscalls/ '+ ' -name enable -exec bash -c "echo 1 > {}" \;')
    call('find ' + tracing_dir + 'events/syscalls/ '+ ' -name enable -exec bash -c "echo 1 > {}" \;', shell=True)
